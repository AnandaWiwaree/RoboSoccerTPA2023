...
float spFli = 155;//พิกัดกึ่งกลางลูกบอลแนวดิ่ง
    ...
    if (discoveState) {
      ...
      if (abs(pvYaw) < alignErrorGap) {
        ...
        if ((abs(160 - ballPosX) < rotErrorGap) && (abs(spFli - ballPosY) < flingErrorGap)) {
          ...
          while (1) {
            getIMU(); heading(100, 90, 0);//ความเร็ว/ทิศทาง/ทิศที่หันหน้าไปตามเข็มทิศ
            if (millis() - loopTimer >= 400)break;//เวลาในการเลี้ยงลูก
          }
          shoot();
          ao();
          delay(500);
